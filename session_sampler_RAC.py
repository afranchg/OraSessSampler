...
# Para no output por pantalla y guardar datos. Modo en background
    if (outp == 'n'):
        stmttrg = "select :mues, sysdate, inst_id, sid, serial#, username, status, machine, program, sql_id, prev_sql_id, decode(blocking_instance,NULL,'NULL',blocking_instance), decode(blocking_session,null,'NULL',blocking_session), event, p1, p2, p3, wait_time, seconds_in_wait, state from dual,GV$SESSION where type = 'USER' and (client_info <> 'SESSION_SAMPLER' or client_info IS NULL) and status = 'ACTIVE'"
        targ = cont.cursor()
        cont.clientinfo = 'SESSION_SAMPLER'
        monc = conr.cursor()
        conr.clientinfo = 'SESSION_SAMPLER'
...
#           stmtmon = 'insert into sesmon values(' + str(elem[0]) + ',' + 'to_date(\'' + str(elem[1]) + '\',\'YYYY-MM-DD HH24:MI:SS\'),' +  str(elem[2]) + ',' + str(elem[3]) +  ',' +  str(elem[4]) + ','  + '\'' + str(elem[5]) +  '\''  +  ',' + '\'' + str(elem[6]) +  '\''  +  ',' + '\'' + str(elem[7]) +  '\''  +  ',' + '\'' + str(elem[8]) +  '\''  +  ',' + '\'' + str(elem[9]) +  '\''  +  ',' +  '\'' + str(elem[10]) + '\'' +  ',' +  '\'' + str(elem[11]) + '\'' +  ',' + '\'' + str(elem[12]) +  '\''  +  ',' + '\'' +  str(elem[13]) + '\'' +  ',' +  str(elem[14]) +  ',' +  str(elem[15]) +  ',' +  str(elem[16]) +  ',' + str(elem[17]) +  ',' +  str(elem[18]) + ',' +  '\''  + str(elem[19])  + '\'' + ')'
#           stmtmon = 'insert into sesmon values(' + str(elem[0]) + ',' + 'to_date(\'' + str(elem[1]) + '\',\'YYYY-MM-DD HH24:MI:SS\'),' +  str(elem[2]) + ',' + str(elem[3]) +  ',' +  str(elem[4]) + ','  + '\'' + str(elem[5]) +  '\''  +  ',' + '\'' + str(elem[6]) +  '\''  +  ',' + '\'' + str(elem[7]) +  '\''  +  ',' + '\'' + str(elem[8]) +  '\''  +  ',' + '\'' + str(elem[9]) +  '\''  +  ',' +  str(elem[10]) +  ',' +  str(elem[11]) +  ',' + '\'' + str(elem[12]) +  '\''  +  ',' +  str(elem[13]) +  ',' +  str(elem[14]) +  ',' +  str(elem[15]) +  ',' +  str(elem[16]) +  ',' + str(elem[17]) +  ',' + '\'' + str(elem[18]) +  '\''  +  ')'
...
##  Si queremos solo output por pantalla y no guardar datos conexion remota y vista creada
       ...
       stmttrg = "select :mues, sysdate, inst_id, sid, serial#, username, status, machine, program, sql_id, decode(blocking_instance,NULL,'NULL',blocking_instance), decode(blocking_session,null,'NULL',blocking_session), event, p1, p2, p3, wait_time, seconds_in_wait, state from dual,GV$SESSION where type = 'USER' and (client_info <> 'SESSION_SAMPLER' or client_info IS NULL) and status = 'ACTIVE'"
...
# Aqui ponemos la conexion local
  ...
  if (lcv == 'y'):
    ...
    try:
        while True:
          sam = con.cursor()
          con.clientinfo = 'SESSION_SAMPLER'
          sam.execute("select sysdate, inst_id, sid, serial#, username, status, machine, substr(program,1,20), sql_id, decode(blocking_instance,NULL,'NULL',blocking_instance), decode(blocking_session,null,'NULL',blocking_session), event, p1, p2, p3, wait_time, seconds_in_wait, state from dual,GV$SESSION where type = 'USER' and (client_info <> 'SESSION_SAMPLER' or client_info IS NULL) and status = 'ACTIVE' ")
          sample = sam.fetchall()
          print("BD Monitorizada: %s" % dbmon)
          print(" %-3s | %-6s | %-7s | %-20s | %-30s | %-20s | %-12s | %-12s | %-15s | %-8s | %-5s " % ("InN","SID", "SERIAL#", "PROGRAM","Evento", "Usuario", "Estado", "Descripcion", "SQL_ID", "BLKSES", "BLKIN"))
          for elem in sample:
              if (elem[17] != 'WAITING'):
                   estado = 'WORKING'
                   event = 'On CPU / runqueue'
                  ...
                  estado = 'WAITING'
                  event =  elem[11]
              print(" %-3s | %-6s | %-7s | %-20s | %-30s | %-20s | %-12s | %-12s | %-15s | %-8s | %-5s " % ( elem[1], elem[2], elem[3], elem[7], event , elem[4], elem[5], estado , elem[8], elem[10], elem[9]))
          sam.close()
          print
          print
          time.sleep(intv)
